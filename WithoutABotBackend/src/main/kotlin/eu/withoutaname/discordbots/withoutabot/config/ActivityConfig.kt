...
class ActivityConfig(
	...
	class Serializer : SimpleSerializer<ActivityConfig> {
		...
		override fun serialize(entity: ActivityConfig?): String {
			return entity?.run { "${type.name} ${name.trim()}" } ?: "DEFAULT nothing"
		}
	...
	class Deserializer : SimpleDeserializer<ActivityConfig> {
		...
		override fun deserialize(source: String?): ActivityConfig {
			return source?.trim()?.let {
				var name = it
